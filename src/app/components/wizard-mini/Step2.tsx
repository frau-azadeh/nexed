import { MiniStep2FormValue, MiniStep2Schema } from '@/validation/wizard-mini.step2';
...
const Step2:React.FC<Props> = ({onBack, onNext, defaultValues}) => {
  ...
  return (
   <form onSubmit={handleSubmit(()=> void goNext(), onError)}
   ...
   >
    ...
    <div className='grid grid-cols-1 gap-1'>
        ...
        <div>
            <Button type='button' onClick={onBack} variant='outline'>قبلی</Button>
        
            <Button loading={isSubmitting} type='submit' variant='primary'>{isSubmitting ? "در حال بررسی" : "مرحله بعدی"}</Button>
        </div>
    </div>
   </form>
  )
