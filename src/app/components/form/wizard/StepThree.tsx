...
import { StepThreeFormValue, StepThreeSchema } from '@/validation/cinema.stepthree.schema'
...
const StepThree:React.FC<Props> = ({onBack, onSuccess, step1, step2}) => {
    ...
    const total =
    PRICE[step2.seatType] * Number(step2.quantity || 0);

  // شبیه‌سازی ارسال نهایی به سرور
  const submitRequest = (payload: { step1: StepOneFormValue; step2: StepTwoFormValue }) =>
    new Promise<{ ok: true }>((resolve) => setTimeout(() => resolve({ ok: true }), 900));

  const onSubmit: SubmitHandler<StepThreeFormValue> = async () => {
    await toast.promise(
      submitRequest({ step1, step2 }),
      {
  ...
  return (
    ...
    >
        <div className='mb-4'>
            ...
            <p className='text-sm text-gray-700'>خلاصه سفارش خود را بررسی و قوانین را بپذیرید</p>
        </div>
<div className='mb-4 space-y-2 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700'>
<div><span className='text-gray-500'>نام : {step1.fullName}</span></div>
<div><span className='text-gray-500'>ایمیل {step1.email}</span></div>
<div className='h-px bg-gray-200 my-1'/>
<div><span className='text-gray-500'>فیلم: {step2.movie}</span></div>
<div><span className='text-gray-500'>سانس: {step2.showtime}</span></div>
<div><span className='text-gray-500'>نوع صندلی: {step2.seatType === "vip" ? "Vip" : "معمولی"}</span></div>
<div><span className='text-gray-500'>تعدا: {step2.quantity}</span></div>
<div><span className='text-gray-500'>جمع کل تومان: {total.toLocaleString("fa-IR")}</span></div>
</div>
<div className='mb-2'>
<Controller
    name="accept"
...
<div className="mt-6 flex items-center gap-3">
    <Button type='button' variant='outline' onClick={onBack}>قبلی</Button>
    <Button type='submit' loading={isSubmitting}>{isSubmitting ? "ثبت نهایی" : "در جال ثبت"}</Button>
</div>
    </form>
  )
}
